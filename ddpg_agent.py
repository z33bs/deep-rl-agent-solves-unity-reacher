...
from ddpg_policy import Actor, Critic
...
WEIGHT_DECAY = 0  # L2 weight decay for Adam optimiser
HIDDEN_DIMS = (256, 128)
...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed, load_file_prefix=None):
        ...
        self.actor_target = Actor(state_size, action_size, random_seed, HIDDEN_DIMS[0], HIDDEN_DIMS[1]).to(device)
        self.actor_optimizer = optim.Adam(self.actor_local.parameters(), lr=LR_ACTOR)

        # Critic Network (w/ Target Network)
        self.critic_local = Critic(state_size, action_size, random_seed, HIDDEN_DIMS[0], HIDDEN_DIMS[1]).to(device)
        self.critic_target = Critic(state_size, action_size, random_seed, HIDDEN_DIMS[0], HIDDEN_DIMS[1]).to(device)
        ...
        if load_file_prefix is not None:
    ...
    def save(self, path='data/ddpg_agent'):
        """path includes prefix of filename, will add _actor.pth and _critic.pth"""
        Path('data').mkdir(parents=True, exist_ok=True)
        torch.save(self.actor_local.state_dict(), path + '_actor.pth')
        torch.save(self.critic_local.state_dict(), path + '_critic.pth')
